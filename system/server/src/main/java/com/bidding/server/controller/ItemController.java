...
public class ItemController {
    ...
    public void handleAddItem(JsonObject request) {
        ...
        try {
            ...
            switch (type) {
                case "Art":
                    newItem = new com.bidding.server.model.item.Art(null, name, desc, price, null, client.getLoggedInUser(), condition, request.get("artist").getAsString(), request.get("medium").getAsString(), request.get("yearCreated").getAsInt(), request.get("dimensions").getAsString());
                    break;
                case "Electronics":
                    newItem = new com.bidding.server.model.item.Electronics(null, name, desc, price, null, client.getLoggedInUser(), condition, request.get("brand").getAsString(), request.get("model").getAsString(), request.get("warrantyMonths").getAsInt(), request.get("powerWatts").getAsInt());
                    break;
                case "Vehicle":
                    newItem = new com.bidding.server.model.item.Vehicle(null, name, desc, price, null, client.getLoggedInUser(), condition, request.get("make").getAsString(), request.get("model").getAsString(), request.get("year").getAsInt(), request.get("mileage").getAsInt(), request.get("fuelType").getAsString());
                    break;
                default:
